...
function checkPassword(form) {
    ...
    if (password !== secondpass || isWeak(password) || password.includes("uoc") || password.includes("helmepa") || password.includes("tuc")) {
        window.scrollTo(0, 0);
        val = false;
    }
    ...
    if (usertype === "Student") {
        ...
        if(val === true) registerStudent();
    }
    else{
        if(val === true) registerLibrarian();
    }
    return val;
}
...
function supervisor() {
    ...
    if (usertype === "Library supervisor") {
        ...
        document.getElementById("libraryinfo").required = false;
    ...
    xhr.addEventListener("readystatechange", function () {
        ...
        if (this.readyState === this.DONE) {
            if (this.responseText !== "{}") {
                ...
                lat = obj[0].lat;
                lon = obj[0].lon;
                test=document.getElementById('lat').value = lat;
                test2=document.getElementById('lon').value = lon;
